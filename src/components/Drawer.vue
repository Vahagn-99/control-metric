<template>
  <div class="max-w-2xl p-4 mx-auto">
    ...
    >
      ...
      <div class="mt-6">
        <div>
          <Input name="specification" label="Имя / Специализация" v-model="employeeStore.employee.specification"/>
        </div>
        <div class="mt-2">
          <label class="dct-label mb-1 text-sm font-medium text-gray-900 dark:text-white" >
            Вопросы
          </label>
          <div>
            <template v-for="(question,key) in questionStore.questions" :key="key">
              <div class="mt-2">
                <div class="flex gap-1">
                  <Input  name="question[]" v-model="questionStore.questions[key].question"/>
                  <div>
                    <button type="button" @click="deleteQuestion(key)" data-modal-target="delete-modal" data-modal-toggle="delete-modal"
          ...
          <div class="mt-2 flex justify-end">
            <button @click="addQuestion" class="dct-button text-white bg-[#4c8bf7] hover:bg-[#5c8bf9] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 flex justify-center items-center gap-1">
...
function deleteQuestion(index){
  if(questionStore.questions[index].id){
    questionStore.deleteQuestion(employeeStore.employee.id,questionStore.questions[index].id)
  }
